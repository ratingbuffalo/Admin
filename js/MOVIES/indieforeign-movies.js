// js/indieforeign-movies.js
document.addEventListener("DOMContentLoaded", () => {
    const moviesGrid = document.querySelector(".indieforeign-grid");
    const pagination = document.querySelector(".indieforeign-pagination");
    let currentPage = 1;
    ...
    async function fetchIndieForeignMovies(pages = 5) {
      let results = [];
      for (let p = 1; p <= pages; p++) {
        // Fetch Indie + Foreign (exclude English)
        const url = `${TMDB_BASE_URL}/discover/movie?api_key=${TMDB_API_KEY}&page=${p}&with_keywords=independent&with_original_language!=en`;
    ...
    async function prepareMovies() {
      const rawMovies = await fetchIndieForeignMovies(10);
  
      // ✅ Filter movies released between 2000–current year
    ...
    async function loadMovies(page = 1) {
      ...
      if (pageMovies.length === 0) {
        moviesGrid.innerHTML = `<p class="error">No Indie & Foreign movies found between 2000–${CURRENT_YEAR}.</p>`;
