// js/documentary-movies.js
document.addEventListener("DOMContentLoaded", () => {
    const moviesGrid = document.querySelector(".documentary-grid");
    const pagination = document.querySelector(".documentary-pagination");
    let currentPage = 1;
    ...
    async function fetchDocumentaryMovies(pages = 5) {
      let results = [];
      for (let p = 1; p <= pages; p++) {
        const url = `${TMDB_BASE_URL}/discover/movie?api_key=${TMDB_API_KEY}&with_genres=99&page=${p}`;
    ...
    async function loadMovies(page = 1) {
      ...
      if (pageMovies.length === 0) {
        moviesGrid.innerHTML = `<p class="error">No Documentary movies found between 2000–${CURRENT_YEAR}.</p>`;
      ...
      for (const movie of pageMovies) {
        ...
        // ✅ Fetch runtime async and remove <40min
        (async () => {
          try {
