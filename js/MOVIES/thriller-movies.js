// js/thriller-movies.js
document.addEventListener("DOMContentLoaded", () => {
    const moviesGrid = document.querySelector(".thriller-grid");
    const pagination = document.querySelector(".thriller-pagination");
    let currentPage = 1;
    const itemsPerPage = 20; // max 20 per page
    ...
    async function fetchThrillerMovies(pages = 5) {
      let results = [];
      for (let p = 1; p <= pages; p++) {
        const url = `${TMDB_BASE_URL}/discover/movie?api_key=${TMDB_API_KEY}&with_genres=53&page=${p}`;
    ...
    async function prepareMovies() {
      const rawMovies = await fetchThrillerMovies(10); // fetch ~200 movies
  
      // ✅ Only years >= 2000 & <= current year
    ...
    async function loadMovies(page = 1) {
      ...
      if (pageMovies.length === 0) {
        moviesGrid.innerHTML = `<p class="error">No Thriller movies found between 2000–${CURRENT_YEAR}.</p>`;
