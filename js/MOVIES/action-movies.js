...
document.addEventListener("DOMContentLoaded", () => {
    const moviesGrid = document.querySelector(".action-grid");
    ...
    async function fetchActionMovies(pages = 5) {
      let results = [];
      for (let p = 1; p <= pages; p++) {
        const url = `${TMDB_BASE_URL}/discover/movie?api_key=${TMDB_API_KEY}&with_genres=28&page=${p}`;
    ...
    async function prepareMovies() {
      const rawMovies = await fetchActionMovies(10); // fetch ~200 movies
    ...
    async function loadMovies(page = 1) {
      ...
      if (pageMovies.length === 0) {
        moviesGrid.innerHTML = `<p class="error">No Action movies found between 2000–${CURRENT_YEAR}.</p>`;
