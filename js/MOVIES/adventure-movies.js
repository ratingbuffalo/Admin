// js/adventure-movies.js
document.addEventListener("DOMContentLoaded", () => {
    const moviesGrid = document.querySelector(".adventure-grid");
    const pagination = document.querySelector(".adventure-pagination");
    let currentPage = 1;
    ...
    async function fetchAdventureMovies(pages = 5) {
      let results = [];
      for (let p = 1; p <= pages; p++) {
        const url = `${TMDB_BASE_URL}/discover/movie?api_key=${TMDB_API_KEY}&with_genres=12&page=${p}`;
    ...
    async function prepareMovies() {
      const rawMovies = await fetchAdventureMovies(10);
    ...
    async function loadMovies(page = 1) {
      ...
      if (pageMovies.length === 0) {
        moviesGrid.innerHTML = `<p class="error">No Adventure movies found between 2000–${CURRENT_YEAR}.</p>`;
