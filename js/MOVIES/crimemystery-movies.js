// js/crimemystery-movies.js
document.addEventListener("DOMContentLoaded", () => {
    const moviesGrid = document.querySelector(".crimemystery-grid");
    const pagination = document.querySelector(".crimemystery-pagination");
    let currentPage = 1;
    ...
    async function fetchCrimeMysteryMovies(pages = 5) {
      let results = [];
      for (let p = 1; p <= pages; p++) {
        const url = `${TMDB_BASE_URL}/discover/movie?api_key=${TMDB_API_KEY}&with_genres=80,9648&page=${p}`;
    ...
    async function prepareMovies() {
      const rawMovies = await fetchCrimeMysteryMovies(10);
  
      // ✅ Keep only movies released between 2000 and current year
    ...
    async function loadMovies(page = 1) {
      ...
      if (pageMovies.length === 0) {
        moviesGrid.innerHTML = `<p class="error">No Crime & Mystery movies found between 2000–${CURRENT_YEAR}.</p>`;
        ...
        (async () => {
          try {
            ...
            if (details && typeof details.runtime === "number" && details.runtime >= 40) {
              ...
              card.remove(); // remove <40min movies
            }
          } catch {
            card.remove();
          }
