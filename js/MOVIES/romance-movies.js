// js/romance-movies.js
document.addEventListener("DOMContentLoaded", () => {
    const moviesGrid = document.querySelector(".romance-grid");
    const pagination = document.querySelector(".romance-pagination");
    let currentPage = 1;
    ...
    async function fetchRomanceMovies(pages = 5) {
      let results = [];
      for (let p = 1; p <= pages; p++) {
        const url = `${TMDB_BASE_URL}/discover/movie?api_key=${TMDB_API_KEY}&with_genres=10749&page=${p}`;
    ...
    async function prepareMovies() {
      const rawMovies = await fetchRomanceMovies(10);
  
      // ✅ Filter valid years 2000–current
    ...
    async function loadMovies(page = 1) {
      ...
      if (pageMovies.length === 0) {
        moviesGrid.innerHTML = `<p class="error">No Romance movies found between 2000–${CURRENT_YEAR}.</p>`;
