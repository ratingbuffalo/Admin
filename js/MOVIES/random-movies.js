...
document.addEventListener("DOMContentLoaded", () => {
    const moviesGrid = document.querySelector(".random-grid");
    const pagination = document.querySelector(".random-pagination");
    let currentPage = 1;
    let totalPages = 20; // let's pretend we have 20 pages of random picks
    const maxVisiblePages = 10;
    const pageSize = 20; // number of random picks per page
    ...
    async function fetchRandomBatch() {
      // Pick a random page between 1 and 500
      const randomPage = Math.floor(Math.random() * 500) + 1;
      const url = `${TMDB_BASE_URL}/discover/movie?api_key=${TMDB_API_KEY}&page=${randomPage}`;
    ...
    async function loadMovies(page = 1) {
      ...
      try {
        const data = await fetchRandomBatch();
        ...
        // Shuffle and slice to page size
        const shuffled = data.results.sort(() => 0.5 - Math.random());
        const picks = shuffled.slice(0, pageSize);
        ...
        console.error(err);
        moviesGrid.innerHTML = `<p class="error">Failed to load random movies.</p>`;
        totalPages = 1;
        renderPagination();
      } finally {
