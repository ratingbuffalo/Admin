...
document.addEventListener("DOMContentLoaded", () => {
    const moviesGrid = document.querySelector(".scifi-grid");
    const pagination = document.querySelector(".scifi-pagination");
    let currentPage = 1;
    ...
    async function fetchSciFiMovies(pages = 5) {
      let results = [];
      for (let p = 1; p <= pages; p++) {
        const url = `${TMDB_BASE_URL}/discover/movie?api_key=${TMDB_API_KEY}&with_genres=878&page=${p}`;
    ...
    async function prepareMovies() {
      const rawMovies = await fetchSciFiMovies(10); // fetch ~200 movies
  
      // ✅ Filter only valid years between 2000–current year
    ...
    async function loadMovies(page = 1) {
      ...
      if (pageMovies.length === 0) {
        moviesGrid.innerHTML = `<p class="error">No Science Fiction movies found between 2000–${CURRENT_YEAR}.</p>`;
