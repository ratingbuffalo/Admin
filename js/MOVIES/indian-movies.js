...
document.addEventListener("DOMContentLoaded", () => {
    const moviesGrid = document.querySelector(".indian-grid");
    const pagination = document.querySelector(".indian-pagination");
    let currentPage = 1;
    ...
    async function fetchIndianMovies(pages = 10) {
      let results = [];
      for (let p = 1; p <= pages; p++) {
        const url = `${TMDB_BASE_URL}/discover/movie?api_key=${TMDB_API_KEY}&with_origin_country=IN&page=${p}`;
        try {
          ...
          console.error(`Failed to fetch Indian movies page ${p}`, err);
        }
      }
      return results.slice(0, MAX_MOVIES);
    }
    ...
    async function prepareMovies() {
      const rawMovies = await fetchIndianMovies(10);
      ...
      allMovies.sort((a, b) => {
        ...
        return yearB - yearA; // newest → oldest
      });
    }
    ...
    async function loadMovies(page = 1) {
      ...
      if (pageMovies.length === 0) {
        moviesGrid.innerHTML = `<p class="error">No Indian movies found between 2000–${CURRENT_YEAR}.</p>`;
