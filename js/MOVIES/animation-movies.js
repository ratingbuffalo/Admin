// js/animation-movies.js
document.addEventListener("DOMContentLoaded", () => {
    const moviesGrid = document.querySelector(".animation-grid");
    const pagination = document.querySelector(".animation-pagination");
    let currentPage = 1;
    const itemsPerPage = 20; // ✅ max 20 cards per page
    ...
    async function fetchAnimationMovies(pages = 5) {
      let results = [];
      for (let p = 1; p <= pages; p++) {
        const url = `${TMDB_BASE_URL}/discover/movie?api_key=${TMDB_API_KEY}&with_genres=16&page=${p}`;
    ...
    async function prepareMovies() {
      const rawMovies = await fetchAnimationMovies(10); // fetch up to ~200 movies
  
      // ✅ Filter valid years, only >= 2000 & <= current year
    ...
    async function loadMovies(page = 1) {
      ...
      if (pageMovies.length === 0) {
        moviesGrid.innerHTML = `<p class="error">No Animation movies found between 2000–${CURRENT_YEAR}.</p>`;
        ...
        (async () => {
          try {
            ...
            if (details && typeof details.runtime === "number" && details.runtime >= 40) {
              ...
              card.remove(); // Remove if runtime < 40min
            }
          } catch {
            card.remove();
          }
    ...
    function renderPagination() {
      ...
      const maxButtons = 10; // ✅ limit number of pagination buttons
