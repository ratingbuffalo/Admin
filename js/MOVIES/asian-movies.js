...
document.addEventListener("DOMContentLoaded", () => {
  const moviesGrid = document.querySelector(".asian-grid");
  const pagination = document.querySelector(".asian-pagination");
  let currentPage = 1;
  const itemsPerPage = 20; // always 20 per page
  const CURRENT_YEAR = new Date().getFullYear();
  const MIN_YEAR = 2000;
  const MAX_MOVIES = 400; // limit total collected
  const MAX_PAGES = 10;   // cap pagination buttons
  let allMovies = [];

  // Subset of Asian countries (ISO codes)
  const asianCountries = [
    "JP", "CN", "KR", "PH", "TH", "ID", "MY", "PK", "BD"
  ];/* "IN",  */
  ...
  async function fetchAsianMovies(pages = 2) {
    let results = [];
  ...
  async function prepareMovies() {
    const rawMovies = await fetchAsianMovies(2); // 2 pages per country
    ...
    allMovies.sort((a, b) => {
      ...
      return yearB - yearA; // sort newest → oldest
    });
  }
  ...
  async function loadMovies(page = 1) {
    ...
    if (pageMovies.length === 0) {
      moviesGrid.innerHTML = `<p class="error">No Asian movies found between 2000–${CURRENT_YEAR}.</p>`;
