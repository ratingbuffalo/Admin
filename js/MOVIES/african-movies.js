// js/african-movies.js
document.addEventListener("DOMContentLoaded", () => {
    const moviesGrid = document.querySelector(".african-grid");
    const pagination = document.querySelector(".african-pagination");
    let currentPage = 1;
    const itemsPerPage = 20; // always 20 per page
    const CURRENT_YEAR = new Date().getFullYear();
    const MIN_YEAR = 2000;
    const MAX_MOVIES = 400; // limit total collected
    const MAX_PAGES = 10;   // limit pagination buttons
    let allMovies = [];
  
    const africanCountries = [
      "ZA","NG","EG","KE","GH","TZ","UG","DZ","MA","SN"
    ]; // subset to avoid overload
    ...
    async function fetchAfricanMovies(pages = 1) {
      let results = [];
  
      for (const country of africanCountries) {
        for (let p = 1; p <= pages; p++) {
          const url = `${TMDB_BASE_URL}/discover/movie?api_key=${TMDB_API_KEY}&with_origin_country=${country}&page=${p}`;
          try {
            ...
            if (data && Array.isArray(data.results)) {
              ...
              if (results.length >= MAX_MOVIES) {
                return results.slice(0, MAX_MOVIES); // stop early
              }
            }
          } catch (err) {
            console.error(`Failed to fetch ${country} page ${p}`, err);
          }
        }
      }
  
      return results.slice(0, MAX_MOVIES);
    }
  
    async function prepareMovies() {
      const rawMovies = await fetchAfricanMovies(2); // fetch only 2 pages per country
      ...
      allMovies.sort((a, b) => {
        ...
        return yearB - yearA; // newer → older
      });
    }
    ...
    async function loadMovies(page = 1) {
      ...
      if (pageMovies.length === 0) {
        moviesGrid.innerHTML = `<p class="error">No African movies found between 2000–${CURRENT_YEAR}.</p>`;
    ...
    function renderPagination() {
      ...
      const cappedPages = Math.min(totalPages, MAX_PAGES); // cap buttons
      ...
      nextBtn.innerHTML = "&gt;";
      nextBtn.disabled = currentPage === totalPages || currentPage >= MAX_PAGES;
