// js/fantasy-movies.js
document.addEventListener("DOMContentLoaded", () => {
    const moviesGrid = document.querySelector(".fantasy-grid");
    const pagination = document.querySelector(".fantasy-pagination");
    let currentPage = 1;
    ...
    async function fetchFantasyMovies(pages = 5) {
      let results = [];
      for (let p = 1; p <= pages; p++) {
        const url = `${TMDB_BASE_URL}/discover/movie?api_key=${TMDB_API_KEY}&with_genres=14&page=${p}`;
    ...
    async function prepareMovies() {
      const rawMovies = await fetchFantasyMovies(10);
    ...
    async function loadMovies(page = 1) {
      ...
      if (pageMovies.length === 0) {
        moviesGrid.innerHTML = `<p class="error">No Fantasy movies found between 2000–${CURRENT_YEAR}.</p>`;
