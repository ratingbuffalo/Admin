...
document.addEventListener("DOMContentLoaded", () => {
    const showsGrid = document.querySelector(".asian-grid");
    ...
    async function fetchAsianTv(pages = 5) {
      let results = [];
      const asianCountries = ["JP", "KR", "CN", "HK", "IN", "TH", "PH", "SG", "MY"]; 
    ...
    async function prepareShows() {
      const rawShows = await fetchAsianTv(5); // fetch multiple countries
    ...
    async function loadShows(page = 1) {
      ...
      if (pageShows.length === 0) {
        showsGrid.innerHTML = `<p class="error">No Asian TV shows found between 2000–${CURRENT_YEAR}.</p>`;
