// js/crime-investigation-tvshows.js
document.addEventListener("DOMContentLoaded", () => {
    const showsGrid = document.querySelector(".crime-investigation-grid");
    const pagination = document.querySelector(".crime-investigation-pagination");
    ...
    async function fetchCrimeTv(pages = 5) {
      let results = [];
  
      for (let p = 1; p <= pages; p++) {
        const url = `${TMDB_BASE_URL}/discover/tv?api_key=${TMDB_API_KEY}&with_genres=80&page=${p}`;
    ...
    async function prepareShows() {
      const rawShows = await fetchCrimeTv(5);
    ...
    async function loadShows(page = 1) {
      ...
      if (pageShows.length === 0) {
        showsGrid.innerHTML = `<p class="error">No Crime & Investigation TV shows found between 2000–${CURRENT_YEAR}.</p>`;
