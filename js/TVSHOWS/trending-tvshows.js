// js/trending-tvshows.js
document.addEventListener("DOMContentLoaded", () => {
    const showsGrid = document.querySelector(".trending-grid");
    const pagination = document.querySelector(".trending-pagination");
    ...
    async function fetchTrendingTv(pages = 5) {
      let results = [];
      for (let p = 1; p <= pages; p++) {
        const url = `${TMDB_BASE_URL}/trending/tv/week?api_key=${TMDB_API_KEY}&page=${p}`;
    ...
    async function prepareShows() {
      const rawShows = await fetchTrendingTv(10); // ~200 shows
    ...
    async function loadShows(page = 1) {
      ...
      if (pageShows.length === 0) {
        showsGrid.innerHTML = `<p class="error">No trending TV shows found between 2000–${CURRENT_YEAR}.</p>`;
