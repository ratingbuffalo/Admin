...
document.addEventListener("DOMContentLoaded", () => {
    const showsGrid = document.querySelector(".drama-grid");
    const pagination = document.querySelector(".drama-pagination");
    ...
    async function fetchDramaTv(pages = 5) {
      let results = [];
  
      for (let p = 1; p <= pages; p++) {
        const url = `${TMDB_BASE_URL}/discover/tv?api_key=${TMDB_API_KEY}&with_genres=18&page=${p}`;
    ...
    async function prepareShows() {
      const rawShows = await fetchDramaTv(5);
    ...
    async function loadShows(page = 1) {
      ...
      if (pageShows.length === 0) {
        showsGrid.innerHTML = `<p class="error">No Drama TV shows found between 2000–${CURRENT_YEAR}.</p>`;
