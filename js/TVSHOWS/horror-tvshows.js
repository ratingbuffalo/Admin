// js/horror-tvshows.js
document.addEventListener("DOMContentLoaded", () => {
    const showsGrid = document.querySelector(".horror-grid");
    const pagination = document.querySelector(".horror-pagination");
    ...
    async function fetchHorrorTv(pages = 5) {
      let results = [];
      for (let p = 1; p <= pages; p++) {
        const url = `${TMDB_BASE_URL}/discover/tv?api_key=${TMDB_API_KEY}&with_genres=27&page=${p}`;
        try {
          ...
          console.warn("fetchHorrorTv page error:", err);
        }
      }
  
      // Deduplicate by id (prevents duplicate cards across pages)
      const seen = new Set();
    ...
    async function loadShows(page = 1) {
      ...
      if (pageShows.length === 0) {
        showsGrid.innerHTML = `<p class="error">No Horror TV shows found between ${MIN_YEAR}–${CURRENT_YEAR}.</p>`;
    ...
    function renderPagination() {
      ...
      const totalPages = Math.max(1, Math.ceil(allShows.length / itemsPerPage));
      const maxButtons = 10;
