// js/recommended-tvshows.js
document.addEventListener("DOMContentLoaded", () => {
    const showsGrid = document.querySelector(".recommended-grid");
    const pagination = document.querySelector(".recommended-pagination");
    ...
    async function fetchRecommendedTv(pages = 5) {
      let results = [];
      for (let p = 1; p <= pages; p++) {
        const url = `${TMDB_BASE_URL}/discover/tv?api_key=${TMDB_API_KEY}&sort_by=vote_average.desc&vote_count.gte=100&page=${p}`;
    ...
    async function prepareShows() {
      const rawShows = await fetchRecommendedTv(10); // ~200 shows
    ...
    async function loadShows(page = 1) {
      ...
      if (pageShows.length === 0) {
        showsGrid.innerHTML = `<p class="error">No recommended TV shows found between 2000–${CURRENT_YEAR}.</p>`;
