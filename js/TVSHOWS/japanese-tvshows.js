// js/japanese-tvshows.js
document.addEventListener("DOMContentLoaded", () => {
    const showsGrid = document.querySelector(".japanese-grid");
    ...
    async function fetchJapaneseTv(pages = 5) {
      let results = [];
  
      for (let p = 1; p <= pages; p++) {
        const url = `${TMDB_BASE_URL}/discover/tv?api_key=${TMDB_API_KEY}&with_origin_country=JP&page=${p}`;
    ...
    async function prepareShows() {
      const rawShows = await fetchJapaneseTv(5);
    ...
    async function loadShows(page = 1) {
      ...
      if (pageShows.length === 0) {
        showsGrid.innerHTML = `<p class="error">No Japanese TV shows found between 2000–${CURRENT_YEAR}.</p>`;
