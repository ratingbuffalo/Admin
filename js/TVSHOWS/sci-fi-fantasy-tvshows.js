// js/sci-fi-fantasy-tvshows.js
document.addEventListener("DOMContentLoaded", () => {
    const showsGrid = document.querySelector(".sci-fi-fantasy-grid");
    const pagination = document.querySelector(".sci-fi-fantasy-pagination");
    ...
    async function fetchSciFiFantasyTv(pages = 5) {
      let results = [];
  
      for (let p = 1; p <= pages; p++) {
        const url = `${TMDB_BASE_URL}/discover/tv?api_key=${TMDB_API_KEY}&with_genres=10765&page=${p}`;
    ...
    async function prepareShows() {
      const rawShows = await fetchSciFiFantasyTv(5);
    ...
    async function loadShows(page = 1) {
      ...
      if (pageShows.length === 0) {
        showsGrid.innerHTML = `<p class="error">No Sci-Fi & Fantasy TV shows found between 2000–${CURRENT_YEAR}.</p>`;
