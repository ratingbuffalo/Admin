// js/african-tvshows.js
document.addEventListener("DOMContentLoaded", () => {
    const showsGrid = document.querySelector(".african-grid");
    ...
    async function fetchAfricanTv(pages = 5) {
      let results = [];
      const africanCountries = ["NG", "ZA", "KE", "EG", "GH", "DZ", "MA", "ET", "TZ"]; 
  
      for (let p = 1; p <= pages; p++) {
        for (const country of africanCountries) {
          const url = `${TMDB_BASE_URL}/discover/tv?api_key=${TMDB_API_KEY}&with_origin_country=${country}&page=${p}`;
    ...
    async function prepareShows() {
      const rawShows = await fetchAfricanTv(5); 
    ...
    async function loadShows(page = 1) {
      ...
      if (pageShows.length === 0) {
        showsGrid.innerHTML = `<p class="error">No African TV shows found between 2000–${CURRENT_YEAR}.</p>`;
