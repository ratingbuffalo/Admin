// js/action-adventure-tvshows.js
document.addEventListener("DOMContentLoaded", () => {
    const showsGrid = document.querySelector(".action-adventure-grid");
    const pagination = document.querySelector(".action-adventure-pagination");
    ...
    async function fetchActionAdventureTv(pages = 5) {
      let results = [];
  
      for (let p = 1; p <= pages; p++) {
        const url = `${TMDB_BASE_URL}/discover/tv?api_key=${TMDB_API_KEY}&with_genres=10759&page=${p}`;
    ...
    async function prepareShows() {
      const rawShows = await fetchActionAdventureTv(5);
    ...
    async function loadShows(page = 1) {
      ...
      if (pageShows.length === 0) {
        showsGrid.innerHTML = `<p class="error">No Action & Adventure TV shows found between 2000–${CURRENT_YEAR}.</p>`;
